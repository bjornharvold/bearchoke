...
function org_apache_cxf_trace(str){
    this.trace = str;
    window.alert(this.trace);
}
...
function org_apache_cxf_getXMLHttpRequest()
{
    ...
    try {
       ...
       //CORS ADDON: XHR or XDR request handler
       /*var RequestHandler = Cxf_Transport();
       RequestHandler.handler = Cxf_Transport_Handler();
       httpRequest = RequestHandler.handler;
        */
       //var RequestHandler = new Cxf_Transport_Cors();
       var RequestHandler = new cxf_cors_request_object();
       RequestHandler.init();
       httpRequest = RequestHandler.handler;
       //window.alert('Cliente tipo = ' + RequestHandler.type);
       if(!httpRequest)
           throw "CXF-UTILS-CORS fails to create CORS XMLHttpRequest object";
    } catch(ex) {
        window.alert('CORS failure:' + ex.description);
    }
    /*
...
function org_apache_cxf_client_request(url, requestXML, method, sync, headers)
{
	...
	try {
		...
		//window.alert('this.req getted: ' + this.req.type);
	} catch (err) {
                this.utils.trace("Error creating XMLHttpRequest: " + err);
		this.req = null;
	}

	if (this.req.handler == null) {
	...
	this.req.handler.open(this.method, this.url, !this.sync);
	...
	if (this.mtomparts.length > 0) {
		...
		var ctHeader = 'multipart/related; start-info="text/xml"; type="application/xop+xml"; boundary="'
				+ mimeBoundary + '"';
		this.req.handler.setRequestHeader("Content-Type", ctHeader);

	} else {
	// for now, assume SOAP 1.1. 1.2 calls for application/xml.
	// also assume we're talking Unicode here.
                //CORS
		//XDR does not support setRequestHeader. Discarting it
                //if(this.req.type != 'XDR')
                    this.req.handler.setRequestHeader("Content-Type", "text/xml;charset=utf-8");
	}
    ...
    if (headers) { // must be array indexed by header field.
		...
		//XDR does not support setRequestHeader. Discarting it
                //if(this.req.type != 'XDR')
                    this.req.handler.setRequestHeader(h, headers[h]);
            }
        }
    }
	...
	//XDR does not support setRequestHeader. Discarting it
       // if(this.req.type != 'XDR'){
            this.req.handler.setRequestHeader("SOAPAction", this.soapAction);
     //   }

	var requester = this; /* setup a closure */

        if(this.req.type != 'MS-XDR'){
            //this.req.handler.onreadystatechange = function() {
            this.req.handler.onload = function() {
		          requester.onReadyState();
            }
        }
        else if(this.req.type == 'MS-XDR'){
            this.req.handler.onload = function() {
		          requester.onReadyState();
            }
        }
        //window.alert('request onreadystatechange = ' + this.req.onreadystatechange);
  ...
  //window.alert('will send data now:\n' + dataToSend);
	this.req.handler.send(dataToSend);
}
...
function org_apache_cxf_parse_multipart_related() {
  if(this.req.type == 'MZ-XHR')
	 var contentType = this.req.handler.getResponseHeader("content-type");
	else
	 var contentType = this.req.handler.contentType;

	window.alert('contenttype = ' + contentType);
...
function org_apache_cxf_client_onReadyState() {
	var req = this.req;
	//only XHR has "readyState" property
  if (req.type == 'MZ-XHR'){
    var ready = req.handler.readyState;
    this.utils.trace("onreadystatechange " + ready);

	  if (ready == this.READY_STATE_DONE) {
		  var httpStatus;
		  try {
			 httpStatus = req.handler.status;
		  } catch (e) {
		  ...
		  if (httpStatus == 200 || httpStatus == 0) {
			 if (this.onsuccess != null) {
				...
				this.onsuccess(this, req.handler.responseXML);
			 }
		  } else {
			 this.utils.trace("onreadystatechange DONE ERROR "
					+ req.handler.getAllResponseHeaders() + " " + req.handler.statusText + " "
					+ req.handler.responseText);
			if (this.onerror != null)
				this.onerror(this);
		  }
	  }
	 }
	 //XDR success/error callbacks
	 else{
		 // the onSuccess function is generated, and picks apart the
		 // response.
		 if (req.handler.contentType != 'text/xml') {
		  if (this.parseMultipartRelated()) {
        this.onsuccess(this, this.mpResponseXML);
				return;
			}
		 ...
		 //Creates data as XML doc, once XDomainRequest does not provide responseXML
     var xmlDoc=new ActiveXObject("Microsoft.XMLDOM");
     xmlDoc.async="false";
     xmlDoc.loadXML(req.handler.responseText);

		 this.onsuccess(this, xmlDoc);
   }
}
